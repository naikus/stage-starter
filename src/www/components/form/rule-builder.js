const stringValue = value => value === null || typeof(value) === "undefined" ? "" : String(value),
    ...
    Rules = {
      ...
      length(value, field) {
        ...
        if(min && len < min) {
          const message =
              this.message || `${field.node.attrs.label} must be a minimum of ${min} characters`;
          return invalid(message);
        }
        if(max && len > max) {
          const message =
              this.message || `${field.node.attrs.label} must be a maxinum of ${max} characters`;
          return invalid(message);
        }
      },
      number(value, field) {
        ...
        if(isNaN(val) || val < min || val > max) {
          const message = this.message || `${field.node.attrs.label} is an invalid number`;
          return invalid(message);
        }
      },
      email(value, field) {
        const reEmail = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-zA-Z]{2,6}(?:\.[a-zA-Z]{2,3})?)$/,
            val = stringValue(value);
        if(!reEmail.test(val)) {
          const message = this.message || `${field.node.attrs.label} must be a valid email`;
          return invalid(message);
        }
      },
      fieldCompare(value, field, fields) {
        ...
        if(other.value !== value) {
          const message = this.message || `${field.node.attrs.label} must be same as ${other.label}`;
          return invalid(message);
        }else {
          other.valid = true;
        }
      ...
      pattern(value, field) {
        ...
        if(!regExp.test(val)) {
          const message = this.message || `${field.node.attrs.label} must match pattern: ${regExp}`;
          return invalid(message);
        }
      }
    };
